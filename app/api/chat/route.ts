...
// 外部公開APIの認証キー（Vercel env: JJ_CHATBOT_API_KEY）
...
function detectAgeCategoryId(text: string): number | null {
  ...
  if (/(子ども|子供|小学生)/i.test(text)) return 166; // 6歳以上
  return null;
}
...
export async function POST(req: Request) {
  ...
  try {
    // x-debug: 1 の時だけ最低限の疎通情報（本番運用では後で消してOK）
    if (req.headers.get("x-debug") === "1") {
      return new Response(
        JSON.stringify({
          ...
          has_api_key: !!JJ_CHATBOT_API_KEY,
    ...
    if (SHOP_TOKEN && titles.length) {
      for (const t of titles.slice(0, 3)) {
        ...
        for (const q of queries) {
          ...
          if (!fallback) fallback = r.items.find((x: any) => x?.is_visible) ?? null;
        }
    ...
    if (pickedNames.length === 0) {
      const ageCat = ageCategoryId ?? null;
      const countCat = countCategoryId ?? null;
